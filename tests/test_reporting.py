...
from visualisation_performances_sportives.analyse_sportive.reporting import (
...
def test_generer_reporting_excel(mock_get_clean_data, tmp_path):
    """Vérifie que l'orchestrateur génère bien"""

    # Préparation d'un faux DataFrame
    mock_get_clean_data.return_value = pd.DataFrame(
        {"annee": [2023, 2024], "trimestre": ["T1", "T2"], "distance_km": [12.0, 15.5]}
    )
