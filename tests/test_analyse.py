...
def test_construire_page_analyse():
    ...
    construire_page_analyse(wb, max_data_row=100, len_y=5, len_t=12, len_d=5, max_pivot=10)
    
    # Vérifications
    assert "Analyse" in wb.sheetnames, "L'onglet 'Analyse' n'a pas été créé."
    
    ws = wb["Analyse"]
    assert ws['A1'].value == "ANALYSE DES PERFORMANCES SPORTIVES", "Le titre principal est incorrect."
