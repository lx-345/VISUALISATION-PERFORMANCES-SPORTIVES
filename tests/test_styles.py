...
from visualisation_performances_sportives.analyse_sportive.styles import (
    appliquer_theme_global,
    get_base_styles,
)
...
def test_get_base_styles():
    ...
    assert len(styles) == 5, (
        "La fonction doit retourner exactement 5 éléments de style."
    )
    assert isinstance(styles[0], PatternFill), (
        "Le premier élément doit être un PatternFill."
    )
    ...
    assert isinstance(styles[3], Alignment), (
        "Le quatrième élément doit être un Alignment."
    )
...
def test_appliquer_theme_global():
    ...
    cellule_a1 = ws["A1"]
    assert cellule_a1.fill.start_color.index == "001E1E1E", (
        "La couleur de fond devrait être 1E1E1E."
    )
