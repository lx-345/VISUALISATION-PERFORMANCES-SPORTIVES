...
def preparer_donnees_agregees(df: pd.DataFrame):
    ...
    """
    if df.empty:
        return pd.DataFrame(columns=['annee', 'distance_km']), pd.DataFrame(columns=['annee', 'trimestre', 'distance_km'])

    df_yearly = df.groupby('annee').sum(numeric_only=True).reset_index()
    
    if 'trimestre' in df.columns:
        df_trimestriel = df.groupby(['annee', 'trimestre']).sum(numeric_only=True).reset_index()
    else:
...
def construire_onglets_back(wb, df: pd.DataFrame):
    ...
    if not df_yearly.empty and 'distance_km' in df_yearly.columns:
        for r in dataframe_to_rows(df_yearly[['annee', 'distance_km']], index=False, header=False):
            ws_back.append(r)
